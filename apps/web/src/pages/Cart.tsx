...
export default function Cart() {
  ...
  const productById = useMemo(() => new Map(products.map((p) => [p.id, toProductView(p)])), [products]);
  const cartProducts = items.map((item) => ({ ...item, product: productById.get(item.productId) })).filter((i) => i.product);
  ...
  const handleCheckout = async () => {
    ...
    setSubmitting(true);
    try {
      await api.checkout({ items: cartProducts.map(({ productId, quantity }) => ({ productId, quantity })) });
      clearCart();
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          {cartProducts.map(({ productId, quantity, product }) => (
            <div key={productId} className="flex gap-4 border rounded-xl p-4 bg-card">
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/products/${productId}`} className="font-display font-semibold hover:text-primary truncate block">{product!.name}</Link>
                <p className="text-sm text-muted-foreground">{product!.category}</p>
                <p className="font-display font-bold text-primary mt-1">${product!.price.toFixed(2)}</p>
              </div>
              <div className="flex flex-col items-end justify-between">
                <Button variant="ghost" size="icon" className="text-destructive" onClick={() => removeItem(productId)}><Trash2 className="w-4 h-4" /></Button>
        ...
        <div className="border rounded-xl p-6 bg-card h-fit sticky top-24">
          ...
          <div className="border-t pt-4 mb-6"><div className="flex justify-between font-display font-bold text-lg"><span>Total</span><span>${total.toFixed(2)}</span></div></div>
          <Button className="w-full" size="lg" onClick={handleCheckout} disabled={submitting}>{submitting ? "Placing order..." : user ? "Place Order" : "Sign in to Checkout"}</Button>
        </div>
      </div>
    </div>
  );
